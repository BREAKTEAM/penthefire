...
from utils import server as svr
from utils import client as clt
from utils import nodpi as ndi
from utils import helper as hlr
import os
import sys
import argparse
...
parser.add_argument('-a', '--attacker', default=False,
                    action="store_true", help='Work in attacker mode (spoofing attack)')
parser.add_argument('-s', '--server', default=False,
                    action="store_true", help='Work in server mode')
parser.add_argument('-c', '--client', default=False,
                    action="store_true", help='Work in client mode')
parser.add_argument('-n', '--nodpi', default=False,
                    action="store_true", help='Work in DPI hiding mode')
parser.add_argument('-d', '--decode', default=False,
                    help='Decode protocol message to extract IP params')
parser.add_argument('-t', '--target', default='192.168.5.5',
                    help='IP address of target attack')
parser.add_argument('-b', '--bind', default='',
                    help='IP address to bind to')
parser.add_argument('-l', '--lport', default='',
...
parser.add_argument('-q', '--queue', default=0,
                    help='Netfilter queue to use in nodpi mode')
...
if args.server == True:
    ...
    if args.helper == 'ftp':
        ...
        if args.helper == 'irc':
            args.lport = ''
        target = svr.client.irc(args.target, int(
            args.lport), int(args.port), verbose=args.verbose)
    elif args.helper == 'ftp6':
        if args.lport == '':
            args.lport = 21
        target = svr.client.ftp6(args.target, int(
            args.lport), int(args.port), verbose=args.verbose)
    else:
    ...
    if args.helper == 'ftp':
        if args.helper == '':
            args.lport = 21
        target = clt.ftp(args.target, int(args.lport),
                         int(args.port), verbose=args.verbose)
    elif args.helper == 'irc':
        if args.lport == '':
            args.lport = 6667
        target = clt.irc(args.target, int(args.lport),
                         int(args.port), verbose=args.verbose)
    elif args.helper == 'ftp6':
        if args.lport == '':
            args.lport = 21
        target = clt.ftp6(args.target, int(args.lport),
                          int(args.port), verbose=args.verbose)
    else:
        sys.exit("Selected protocol is currently unsupported")
elif args.attacker == True:
    ...
    if not os.geteuid() == 0:
        ...
        sys.exit(1)
    if args.helper == 'ftp':
        target = hlr.ftp(args.iface, args.target, int(
            args.port), verbose=args.verbose)
    elif args.helper == 'irc':
        target = hlr.irc(args.iface, args.target, int(
            args.port), verbose=args.verbose)
    elif args.helper == 'ftp6':
        target = hlr.ftp6(args.iface, args.target, int(
            args.port), verbose=args.verbose)
    else:
        sys.exit("Selected protocol is currently unsupported")
